...
-- ==========================================
-- ✅ سیستم چرخ پول برای همه بانک‌ها - سرور
-- ==========================================

-- State tracking برای چرخ‌ها
Config.BankTrolleyState = {}

-- Initialize state
for bankKey in pairs(Config.BankTrolleys) do
    Config.BankTrolleyState[bankKey] = {
        spawned = false,
        isGrabbing = {},
        rewardTypes = {}
    }
    for trolleyId in pairs(Config.BankTrolleys[bankKey].trolleys) do
        Config.BankTrolleyState[bankKey].isGrabbing[trolleyId] = false
        Config.BankTrolleyState[bankKey].rewardTypes[trolleyId] = nil
    end
end

-- ریست کردن state چرخ‌ها برای یک بانک
local function ResetBankTrolleyState(bankKey)
    if not Config.BankTrolleyState[bankKey] then return end
    Config.BankTrolleyState[bankKey].spawned = false
    for trolleyId in pairs(Config.BankTrolleys[bankKey].trolleys) do
        Config.BankTrolleyState[bankKey].isGrabbing[trolleyId] = false
        Config.BankTrolleyState[bankKey].rewardTypes[trolleyId] = nil
    end
...
-- تابع تعیین نوع پاداش
local function DetermineTrolleyRewardType(trolley)
    local rand = math.random(1, 100)
    local cumulative = 0
    
    cumulative = cumulative + trolley.emptyChance
    if rand <= cumulative then return 'empty' end
    
    cumulative = cumulative + trolley.diamondChance
    if rand <= cumulative then return 'diamond' end
    
    cumulative = cumulative + trolley.goldChance
    if rand <= cumulative then return 'gold' end
    
    return 'cash'
end

-- Spawn چرخ‌ها برای همه
RegisterNetEvent('qb-bankrobbery:server:spawnBankTrolleys', function(bankKey)
    if not Config.BankTrolleys[bankKey] then return end
    if Config.BankTrolleyState[bankKey].spawned then return end
    
    -- چک وضعیت بانک
    local bankType, bankId = bankKey:match("^(.-)_(.+)$")
    if bankType == "small" then
        bankId = tonumber(bankId)
        if not Config.SmallBanks[bankId] or not Config.SmallBanks[bankId]["isOpened"] then return end
    else
        if not Config.BigBanks[bankKey] or not Config.BigBanks[bankKey]["isOpened"] then return end
    end
    
    Config.BankTrolleyState[bankKey].spawned = true
    
    -- تعیین نوع پاداش برای هر چرخ
    for trolleyId, trolley in pairs(Config.BankTrolleys[bankKey].trolleys) do
        Config.BankTrolleyState[bankKey].rewardTypes[trolleyId] = DetermineTrolleyRewardType(trolley)
    end
    
    -- ارسال به همه کلاینت‌ها
    TriggerClientEvent('qb-bankrobbery:client:spawnBankTrolleys', -1, bankKey, Config.BankTrolleyState[bankKey].rewardTypes)
end)

-- Despawn چرخ‌ها برای همه
RegisterNetEvent('qb-bankrobbery:server:despawnBankTrolleys', function(bankKey)
    if not Config.BankTrolleys[bankKey] then return end
    if not Config.BankTrolleyState[bankKey].spawned then return end
    
    Config.BankTrolleyState[bankKey].spawned = false
    TriggerClientEvent('qb-bankrobbery:client:despawnBankTrolleys', -1, bankKey)
end)

-- Callback برای تعیین نوع پاداش
QBCore.Functions.CreateCallback('qb-bankrobbery:server:getTrolleyRewardType', function(source, cb, bankKey, trolleyId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return cb(nil) end
    
    if not Config.BankTrolleys[bankKey] then return cb(nil) end
    if not Config.BankTrolleys[bankKey].trolleys[trolleyId] then return cb(nil) end
    
    -- چک فاصله
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local trolleyCoords = Config.BankTrolleys[bankKey].trolleys[trolleyId].coords
    if #(coords - trolleyCoords) > 3.0 then
        return cb(nil)
    end
    ...
    if bankType == "small" then
        bankId = tonumber(bankId)
        if not Config.SmallBanks[bankId] or not Config.SmallBanks[bankId]["isOpened"] then return cb(nil) end
    else
        if not Config.BigBanks[bankKey] or not Config.BigBanks[bankKey]["isOpened"] then return cb(nil) end
    end
    
    -- چک اینکه کسی داره loot می‌کنه
    if Config.BankTrolleyState[bankKey].isGrabbing[trolleyId] then
        return cb(nil)
    end
    
    -- چک وجود آیتم bag
    if not Player.Functions.GetItemByName(Config.TrolleyReward.bagItem) then
        return cb(nil)
    end
    
    -- علامت‌گذاری که کسی داره loot می‌کنه
    Config.BankTrolleyState[bankKey].isGrabbing[trolleyId] = true
    
    -- برگرداندن نوع پاداش
    cb(Config.BankTrolleyState[bankKey].rewardTypes[trolleyId])
end)

-- Event برای loot کردن چرخ
RegisterNetEvent('qb-bankrobbery:server:lootBankTrolley', function(bankKey, trolleyId, rewardType)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    
    if not Config.BankTrolleys[bankKey] then return end
    if not Config.BankTrolleys[bankKey].trolleys[trolleyId] then return end
    ...
    -- چک وجود آیتم bag
    if not Player.Functions.GetItemByName(Config.TrolleyReward.bagItem) then
        TriggerClientEvent('QBCore:Notify', src, 'You need a bag to collect the loot!', 'error')
        return
    end
    
    -- مصرف آیتم bag
    Player.Functions.RemoveItem(Config.TrolleyReward.bagItem, 1)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Config.TrolleyReward.bagItem], 'remove')
    
    -- علامت‌گذاری به عنوان loot شده
    Config.BankTrolleyState[bankKey].isGrabbing[trolleyId] = false
    
    -- دادن پاداش بر اساس نوع
    local trolley = Config.BankTrolleys[bankKey].trolleys[trolleyId]
    
    if rewardType == 'cash' then
        local cashAmount = math.random(trolley.cash.min, trolley.cash.max)
        local info = { worth = cashAmount }
        Player.Functions.AddItem('markedbills', 1, false, info)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['markedbills'], 'add')
        TriggerClientEvent('QBCore:Notify', src, 'You collected $' .. cashAmount .. ' from the trolley!', 'success')
        print('[qb-bankrobbery] Player ' .. GetPlayerName(src) .. ' looted ' .. bankKey .. ' trolley ' .. trolleyId .. ' for $' .. cashAmount)
        
    elseif rewardType == 'gold' then
        local goldAmount = math.random(trolley.gold.min, trolley.gold.max)
        Player.Functions.AddItem('goldbar', goldAmount)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['goldbar'], 'add')
        TriggerClientEvent('QBCore:Notify', src, 'You collected ' .. goldAmount .. ' gold bars from the trolley!', 'success')
        print('[qb-bankrobbery] Player ' .. GetPlayerName(src) .. ' looted ' .. bankKey .. ' trolley ' .. trolleyId .. ' for ' .. goldAmount .. ' gold bars')
        
    elseif rewardType == 'diamond' then
        local diamondAmount = math.random(trolley.diamond.min, trolley.diamond.max)
        ...
        TriggerClientEvent('QBCore:Notify', src, 'You collected ' .. diamondAmount .. ' diamonds from the trolley!', 'success')
        print('[qb-bankrobbery] Player ' .. GetPlayerName(src) .. ' looted ' .. bankKey .. ' trolley ' .. trolleyId .. ' for ' .. diamondAmount .. ' diamonds')
        
    elseif rewardType == 'empty' then
        TriggerClientEvent('QBCore:Notify', src, 'The trolley is empty!', 'error')
        print('[qb-bankrobbery] Player ' .. GetPlayerName(src) .. ' looted ' .. bankKey .. ' trolley ' .. trolleyId .. ' but it was empty')
    end
    
    -- ارسال به همه کلاینت‌ها
    TriggerClientEvent('qb-bankrobbery:client:trolleyLooted', -1, bankKey, trolleyId)
end)

-- ریست چرخ‌ها وقتی بانک reset می‌شه
AddEventHandler('qb-bankrobbery:server:setTimeout', function()
    for bankKey in pairs(Config.BankTrolleys) do
        ResetBankTrolleyState(bankKey)
    end
end)

AddEventHandler('qb-bankrobbery:server:SetSmallBankTimeout', function(bankId)
    local bankKey = "small_" .. bankId
    ResetBankTrolleyState(bankKey)
end)
