...
local function OpenPaletoDoor()
    ...
    if object ~= 0 then
        CreateThread(function()
            local targetHeading = Config.BigBanks["paleto"]["heading"].open
            local rotationSpeed = 0.05
            local rotationDelay = 30
            while true do
    ...
    TriggerServerEvent('qb-bankrobbery:server:spawnBankTrolleys', 'paleto')
end
...
local function OpenPacificDoor()
    ...
    if object ~= 0 then
        CreateThread(function()
            local targetHeading = Config.BigBanks["pacific"]["heading"].open
            local rotationSpeed = 0.05
            local rotationDelay = 30
            while true do
                local heading = GetEntityHeading(object)
    ...
    TriggerServerEvent('qb-bankrobbery:server:spawnBankTrolleys', 'pacific')
end
...
local function OpenBankDoor(bankId)
    local object = GetClosestObjectOfType(
        Config.SmallBanks[bankId]["coords"]["x"],
        Config.SmallBanks[bankId]["coords"]["y"],
        Config.SmallBanks[bankId]["coords"]["z"],
        5.0,
        Config.SmallBanks[bankId]["object"],
        false, false, false
    )
    
    if object ~= 0 then
        CreateThread(function()
            local targetHeading = Config.SmallBanks[bankId]["heading"].open
            local rotationSpeed = 0.05
            local rotationDelay = 30
    ...
    TriggerServerEvent('qb-bankrobbery:server:spawnBankTrolleys', 'small_' .. bankId)
end
    ...
    if bankId == "paleto" then
        ...
        if hasItem then
            ...
            QBCore.Functions.Progressbar("open_locker_drill", Lang:t("general.breaking_open_safe"), math.random(18000, 30000), false, true, {
                ...
                SetTimeout(500, function() IsDrilling = false end)
            end, function()
                StopAnimTask(ped, "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                DetachEntity(DrillObject, true, true)
                ...
                if not Config.SmallBanks[closestBank]["isOpened"] then
                    ...
                    if hasItem then
                        ...
                        QBCore.Functions.Progressbar("hack_gate", Lang:t("general.connecting_hacking_device"), math.random(5000, 10000), false, true, {
                            ...
                            local success = exports['qrt-minigame']:Chopping(12, 30)
                            if success then
...
RegisterNetEvent('qb-bankrobbery:client:ResetFleecaLockers', function(BankId)
    ...
    TriggerServerEvent('qb-bankrobbery:server:despawnBankTrolleys', 'small_' .. BankId)
end)
...
CreateThread(function()
    for i = 1, #Config.SmallBanks do
        local bankId = i
        local bankZone = BoxZone:Create(Config.SmallBanks[bankId]["coords"], 1.0, 1.0, {
            name = 'fleeca_'..bankId..'coords_electronickit',
            heading = Config.SmallBanks[bankId]["heading"].closed,
            minZ = Config.SmallBanks[bankId]["coords"].z - 1,
            maxZ = Config.SmallBanks[bankId]["coords"].z + 1,
            debugPoly = false
        })
        
        bankZone:onPlayerInOut(function(inside)
            inElectronickitZone = inside
            if inside and not Config.SmallBanks[bankId]["isOpened"] then
                Config.ShowRequiredItems({
                    [1] = {name = QBCore.Shared.Items["electronickit"]["name"], image = QBCore.Shared.Items["electronickit"]["image"]},
                    [2] = {name = QBCore.Shared.Items["trojan_usb"]["name"], image = QBCore.Shared.Items["trojan_usb"]["image"]}
                }, true)
        ...
        for k in pairs(Config.SmallBanks[bankId]["lockers"]) do
            ...
            if Config.UseTarget then
                exports['qrt-target']:AddBoxZone('fleeca_'..bankId..'coords_locker_'..lockerId, Config.SmallBanks[bankId]["lockers"][lockerId]["coords"], 1.0, 1.0, {
                    name = 'fleeca_'..bankId..'coords_locker_'..lockerId,
                    heading = Config.SmallBanks[bankId]["heading"].closed,
                    minZ = Config.SmallBanks[bankId]["lockers"][lockerId]["coords"].z - 1,
                    maxZ = Config.SmallBanks[bankId]["lockers"][lockerId]["coords"].z + 1,
                    debugPoly = false
                }, {
                    options = {
                        {
                            action = function() openLocker(closestBank, lockerId) end,
                            canInteract = function()
                                return closestBank ~= 0 and not IsDrilling and Config.SmallBanks[bankId]["isOpened"] and not Config.SmallBanks[bankId]["lockers"][lockerId]["isOpened"] and not Config.SmallBanks[bankId]["lockers"][lockerId]["isBusy"]
                            end,
                ...
                local lockerZone = BoxZone:Create(Config.SmallBanks[bankId]["lockers"][lockerId]["coords"], 1.0, 1.0, {
                    name = 'fleeca_'..bankId..'coords_locker_'..lockerId,
                    heading = Config.SmallBanks[bankId]["heading"].closed,
                    minZ = Config.SmallBanks[bankId]["lockers"][lockerId]["coords"].z - 1,
                    maxZ = Config.SmallBanks[bankId]["lockers"][lockerId]["coords"].z + 1,
                ...
                lockerZone:onPlayerInOut(function(inside)
                    if inside and closestBank ~= 0 and not IsDrilling and Config.SmallBanks[bankId]["isOpened"] and not Config.SmallBanks[bankId]["lockers"][lockerId]["isOpened"] and not Config.SmallBanks[bankId]["lockers"][lockerId]["isBusy"] then
                        exports['qb-core']:DrawText(Lang:t("general.break_safe_open_option_drawtext"), 'right')
                        currentLocker = lockerId
                    else
                        if currentLocker == lockerId then
                            currentLocker = 0
    ...
    if not Config.UseTarget then
        while true do
            ...
            if isLoggedIn then
                for i = 1, #Config.SmallBanks do
                    local bankId = i
                    if currentLocker ~= 0 and not IsDrilling and Config.SmallBanks[bankId]["isOpened"] and not Config.SmallBanks[bankId]["lockers"][currentLocker]["isOpened"] and not Config.SmallBanks[bankId]["lockers"][currentLocker]["isBusy"] then
                        sleep = 0
