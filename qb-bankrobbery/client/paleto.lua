...
-- ✅ فقط مینی‌گیم رو اجرا کن، باز کردن در از doors.lua انجام میشه
local function OpenPaletoDoor()
    -- First run Chopping minigame
    local success = exports['qrt-minigame']:Chopping(12, 30)
    if not success then
        QBCore.Functions.Notify("Failed to hack the lock!", "error")
        return false
    end
    
    -- ✅ فقط event رو trigger کن، در از doors.lua باز میشه
    TriggerEvent('qb-bankrobbery:client:openPaletoDoor')
            ...
            if CurrentCops >= Config.MinimumPaletoPolice then
                if not Config.BigBanks["paleto"]["isOpened"] then
                    ...
                    QBCore.Functions.Progressbar("security_pass", Lang:t("general.validating_bankcard"), math.random(5000, 10000), false, true, {
                        ...
                        -- Run minigame and open door
                        local success = OpenPaletoDoor()
                        if success then
...
CreateThread(function()
    ...
    for k in pairs(Config.BigBanks["paleto"]["lockers"]) do
        if Config.UseTarget then
            exports['qrt-target']:AddBoxZone('paleto_coords_locker_'..k, Config.BigBanks["paleto"]["lockers"][k]["coords"], 1.0, 1.0, {
                ...
                options = {
                    {
                        action = function() openLocker("paleto", k) end,
    ...
    -- DrawText for Lockers (non-Target)
